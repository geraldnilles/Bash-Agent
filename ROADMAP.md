...
## ✅ Completed
- **Model Specific Context Limit:** the per-session context ceiling is now derived from the OpenRouter model's `context_length` in TRUE TOKENS (`context_limit_tokens = int(context_length / 4)`) instead of a characters/8 heuristic. `bash_agent.tokenizer.count_tokens` uses the model's real DeepSeek tokenizer for all local accounting; `Agent` queries `/api/v1/models` once at startup, resolves `context_limit`, and passes that ceiling into `ContextManager`, which honors it for the SCRATCHPAD warning threshold and 80% hysteresis trim. On API failure or model missing from the catalog it gracefully falls back to `config.CONTEXT_LIMIT` (327,680 tokens).
