...
By default the agent only uses 25% of the model's reported context window to leave generous headroom. Conversation accounting is done in real tokens using an **offline, model-aware** local estimate (`deepseek/...` models are counted with the exact bundled DeepSeek BPE; OpenAI-family slugs like `gpt-4o`/`openai/gpt-4o` with tiktoken's `o200k`-family encoders; Gemini slugs through an `o200k_base` proxy for the not-open 256k-vocab SentencePiece; any other slug through the provider-neutral `litellm.token_counter`, which itself degrades to a bundled tiktoken BPE; and finally a ~3.5 chars/token heuristic). Pruning / warnings are therefore accurate and track the model you actually run. Tune that budget with `--token-budget`, as an exact token count or a percentage of the model's full window:
